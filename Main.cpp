...
DWORD GetTargetThreadIDFromProcName(const wchar_t * ProcName)
{
	...
	while (retval)
	{
		...
		{
			char buf[MAX_PATH] = { 0 };
			GetFullPathNameA("OKPOE.dll", MAX_PATH, buf, NULL);
...
BOOL AttemptMulti(LPSTR MutexName)
{
	...
	if (hMutex != NULL)
	{
		DWORD pID = FindProcessId(L"PathOfExile.exe");
		printf("pID: %d\n", pID);
...
int main(int argc, char** argv)
{
	if (argc < 2)
	{
		printf("Run as... ./StealthInjector <pid> <dllname>\n");
		exit(0);
	}
	...
	{
		// Retrieve process ID 
		DWORD pID = GetTargetThreadIDFromProcName(L"NOTEPAD.EXE");
